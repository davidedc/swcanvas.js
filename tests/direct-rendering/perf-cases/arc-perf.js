/**
 * Parametric Arc Performance Tests
 *
 ...
 * - 4 arc angle categories (angS-angXL)
 ...
 * - Fill: Arc/pie fill algorithm
 * - sw0: No-op (skipped for stroke)
 * - sw1px: Bresenham arc stroke
 * - swXXS+: Thick arc stroke algorithms
...
registerParametricPerfTests({
    ...
    includeArcAngles: true,

    /**
     * Draw function for arc performance tests.
     ...
     * @param {Object} params - { sizeSequence, strokeSequence, angleSequence, fixedStrokeWidth, operation }
     */
    drawFunction: function(ctx, instances, params) {
        const canvasWidth = ctx.canvas.width;
        const canvasHeight = ctx.canvas.height;
        const { sizeSequence, strokeSequence, angleSequence, fixedStrokeWidth, operation } = params;
        ...
        for (let i = 0; i < numToDraw; i++) {
            ...
            const arcAngle = angleSequence[idx];
            ...
            // Random start angle (also seeded for reproducibility)
            const startAngle = SeededRandom.getRandom() * Math.PI * 2;
            const endAngle = startAngle + arcAngle;

            // Draw fill first, then stroke (standard Canvas order)
            if (hasFill) {
                ctx.fillArc(cx, cy, radius, startAngle, endAngle, false);
            }
            if (hasStroke) {
                // Use outerStrokeArc for stroked arcs (matches SWCanvas API)
                if (ctx.outerStrokeArc) {
                    ctx.outerStrokeArc(cx, cy, radius, startAngle, endAngle, false);
                } else {
                    // Fallback for HTML5 Canvas compatibility
                    ctx.beginPath();
