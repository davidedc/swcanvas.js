/**
 * Parametric Rotated Rectangle Performance Tests
 *
 ...
 * - Fill: RectOpsRot rotated rect fill (DDA/scanline algorithms)
 * - sw0: No-op (skipped for stroke)
 * - sw1px: Rotated Bresenham rectangle stroke
 * - swXXS+: Rotated thick stroke algorithms via RectOpsRot
 * - Semi-transparent: Alpha compositing code paths
 *
 * Rotated means a rotation transform is applied before drawing.
 * This tests the RectOpsRot code path (DDA/scanline algorithms).
 */

registerParametricPerfTests({
    baseId: 'rect-rot-perf',
    baseName: 'Rect Rot',
    category: 'rects',
    operations: [
        'stroke-opaque',
    ...
    /**
     * Draw function for rotated rectangle performance tests.
    ...
    drawFunction: function(ctx, instances, params) {
        ...
        for (let i = 0; i < numToDraw; i++) {
            ...
            const margin = Math.max(50, diagonal / 2 + maxStrokeWidth);
