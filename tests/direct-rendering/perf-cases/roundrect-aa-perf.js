/**
 * Parametric Axis-Aligned Rounded Rectangle Performance Tests
 *
 ...
 * - Fill: RoundedRectOpsAA fill algorithm (fast nested loops)
 * - sw0: No-op (skipped for stroke)
 * - sw1px: Bresenham rounded rect stroke
 * - swXXS+: Thick stroke algorithms via RoundedRectOpsAA
 * - Semi-transparent: Alpha compositing code paths
 *
 * Axis-aligned means no rotation transform is applied.
 * This tests the RoundedRectOpsAA code path.
 */

registerParametricPerfTests({
    baseId: 'roundrect-aa-perf',
    baseName: 'RoundRect AA',
    ...
    /**
     * Draw function for axis-aligned rounded rectangle performance tests.
    ...
    drawFunction: function(ctx, instances, params) {
        ...
        for (let i = 0; i < numToDraw; i++) {
            ...
            const margin = Math.max(50, size / 2 + maxStrokeWidth);
            ...
            if (hasFill) {
                ctx.fillRoundRect(rectX, rectY, width, height, cornerRadius);
            }
            if (hasStroke) {
                ctx.strokeRoundRect(rectX, rectY, width, height, cornerRadius);
            }
        }
