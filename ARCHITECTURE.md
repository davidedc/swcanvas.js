...
- **The fast path requires `pixelDensity === 1`** in addition to identity-like transform. At density > 1, `drawTextFromAtlas` multiplies x/y by N internally; a fast-path blit against the unscaled main surface would land text at wrong logical coords AND wrong size. The slow path's `box × density` intermediate is the only path that preserves logical coordinates under HiDPI.
