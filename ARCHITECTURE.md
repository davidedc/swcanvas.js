...
**Fast path (axis-aligned, uniform scale == atlas density).** When the current transform is an axis-aligned uniform scale whose factor equals the font's `pixelDensity` — with an integer device-pixel translation (`a = d = dpr`, `b = c = 0`, integer `e/f`) — the renderer bakes the translation back into the coordinates (dividing by `dpr`, since `BitmapText` re-multiplies) and calls `BitmapText.drawTextFromAtlas` against the main surface directly. BitmapText resets the transform to identity internally and applies `pixelDensity` itself by scaling coords/glyphs by `dpr`, so a matching geometric scale is reproduced for free — no intermediate buffer. The original density-1 case (pure integer translation) is just `dpr = 1`.

**Slow path (rotation, skew, or a scale ≠ density).** The renderer calls `BitmapText.computeInkBoundingBox(text, x, y, fontProps, textProps)` to get the exact ink rect in CSS pixels, allocates an intermediate `SWCanvasElement` sized to `ceil(box.{width,height} × pixelDensity)`, draws into it via `drawTextFromAtlas` at `(x − box.x, y − box.y)`, then blits the intermediate via `coreCtx.drawImage(...)` with the user's current transform. This reuses the existing transformed-`drawImage` rasterizer rather than introducing a new direct-rendering hot path, so rotation and scale "just work" without per-feature plumbing. It is also the only path that can honour rotation/skew (BitmapText draws axis-aligned only) or a scale that differs from the density — confining the `×dpr` sampling to the intermediate keeps density a sampling detail rather than a layout multiplier.

`drawTextFromAtlas` paints placeholder rectangles for any glyph whose atlas is missing rather than throwing — the same contract holds across BitmapText's colored-batched and per-glyph branches. This means `ctx.fillText(...)` is safe to call before atlases finish loading; it simply produces no visible glyph pixels until then. Glyphs absent from the font's character set (e.g. `°`, `×` outside Arial's atlas) likewise render as per-character placeholder rectangles, leaving surrounding glyphs intact. Sub-9px sizes always render as placeholder rectangles (BitmapText's rasterization floor — atlases below 9px don't exist; metrics interpolate from size 9 so layout still flows). The LRU demo at `examples/text-lru-atlas-demo.html` exercises this end-to-end with dynamic atlas loading bounded by an LRU cache.

### Design invariants

These are load-bearing — read before touching the text pipeline.

- **`BootstrapText.initialize` runs as the last executable line of `dist/swcanvas.js`** (build.sh Phase 4.7). It calls `BitmapText.configure({ canvasFactory: SWCanvas.createCanvas, atlasFormat: 'imageData' })`. The `setAtlasFormat` contract requires that call BEFORE the first `loadFont` / `registerAtlas` — guaranteed by construction for any SWCanvas consumer. Bypassing the dist (e.g. loading individual `src/` files) means manually calling `BitmapText.configure(...)` first.
- **`atlasFormat: 'imageData'`** makes BitmapText decode atlases to `{width, height, data: Uint8ClampedArray}` at load time. `Core.drawImage` requires that shape; the default `'imageElement'` (HTMLImageElement / canvas-mock) would be rejected with "ImageLike must have numeric width and height properties". Standalone BitmapText users keep the default — no impact on them.
- **`BitmapText` is exposed on `globalThis` at the end of the dist IIFE.** The dynamically-injected asset scripts (`metrics-bundle.js`, `positioning-bundle-density-*.js`, `atlas-*-webp.js`) execute in the page's global scope and call `BitmapText.rBundle(...)`, `.pBundle(...)`, `.a(...)`. They cannot reach into the IIFE; the global re-exposure is mandatory. Don't strip it.
- **Embedder HTML pages MUST declare `<meta charset="UTF-8">`.** The vendored BitmapText source contains UTF-8 character literals in its character sets (`'☺☹♠…'`, `'≠'`, `'…'`, `'€'`, `'−'`, `'█'`). Without the charset declaration some browsers default to Latin-1, decoding each UTF-8 byte as a separate character; `MetricsExpander.expand` then throws `Char stream length 885 does not match 895`. The bundled demo HTML has the declaration — keep it in any new consumer page.
- **Gradient and Pattern fillStyle fall back to opaque black** (with a `console.warn`) for text. BitmapText only accepts a single CSS color string. The mapping is `Color → CSS hex/rgba`; anything else goes through `FillStyleToTextColor` → `'#000000'`.
- **`textAlign='start'` and `'end'` map to `'left'` and `'right'`.** SWCanvas does not implement RTL; `ctx.direction='rtl'` is accepted by the setter and snapshotted through `save()`/`restore()`, but has no effect on rendering.
- **`BitmapText.measureText` is left-anchored regardless of `textAlign`.** For an ink rect that accounts for the current align/baseline (e.g. to size an intermediate buffer), use `BitmapText.computeInkBoundingBox(text, x, y, fontProps, textProps)` — that's what the slow path does.
- **The fast path requires the transform's uniform scale to equal `pixelDensity`** (`a = d = dpr`, `b = c = 0`, integer `e/f`); the density-1 identity-translate case is just `dpr = 1`. `drawTextFromAtlas` resets the transform to identity and applies `pixelDensity` itself by multiplying coords/glyph size by N, so a direct blit against the main surface is correct exactly when the geometric scale already equals N (the internal ×N reproduces it). Any rotation, skew, or a scale that differs from the density (e.g. `scale(2,2)` at density 1, or `scale(2,2)` with `textPixelDensity = 1`) falls to the slow path, whose `box × density` intermediate is the only thing that can rotate/skew or hold a scale ≠ the atlas density.
- **Don't patch `vendor/bitmaptext/` directly.** It's gitignored and regenerated from `vendor/bitmaptext.pin`. Upstream patches + pin bumps; see `vendor/bitmaptext.UPDATE.md`.
